...
serve(async (req) => {
  ...
  try {
    ...
    // Default stays on Flash to keep ~$0.003/req baseline.
    const qLen = (query || "").length;
    const historyTurns = Array.isArray(history) ? history.length : 0;
    const COMPLEX_RE = /\b(itin[ée]raire|planifi|organise|propose[- ]?moi|recommande[- ]?moi|conseille[- ]?moi|compare|versus|vs\b|meilleur|top\s*\d|journ[ée]e|week[- ]?end|s[ée]jour|programme|sur\s+\d+\s*jours?|romantique|en\s+famille|avec\s+enfants?|pour\s+(?:un|une|mon|ma|mes)|budget|luxe|authentique|insolite|secret|cach[ée]|hors[- ]des[- ]sentiers|et\s+(?:aussi|ensuite|apr[èe]s|avant)|d'?abord.+(?:ensuite|puis))/i;
    const isComplex = COMPLEX_RE.test(query || "") || qLen >= 110 || historyTurns >= 4;
    const model = (mode || focusHoursPlaceholder()) ? baseModel : (isComplex ? proModel : baseModel);
    function focusHoursPlaceholder() { return false; } // hoists; real focusHours computed later (already on Flash semantics)
