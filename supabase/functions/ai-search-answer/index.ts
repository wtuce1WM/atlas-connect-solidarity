    ...
    try {
      ...
      if (checkIn && checkOut && hasLodgingContext) {
        ...
        if (matched.length > 0) {
          ...
          hotelAvailabilityInstruction = `\n- DISPONIBILITÉ HÔTELS (RÈGLE STRICTE) : L'utilisateur demande des hôtels disponibles du ${checkIn} au ${checkOut} pour ${adults} adulte${adults > 1 ? "s" : ""}. Cite EXCLUSIVEMENT les hôtels listés dans la section "DISPONIBILITÉ HÔTELS" ci-dessous, et pour chacun indique son prix par nuit en gras (ex. **120 EUR/nuit**). INTERDICTION ABSOLUE de mentionner, recommander ou citer d'autres établissements (hôtels, riads, restaurants, lieux à visiter, pépites architecturales, musées, etc.) — ils n'ont pas de disponibilité confirmée et ne sont PAS pertinents pour cette demande. Pas de section "en complément", pas de "à découvrir aussi", pas de suggestions annexes. Termine simplement par une invitation à élargir les dates ou consulter les fiches si besoin.`;
        } else {
