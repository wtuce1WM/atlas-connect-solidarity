    ...
    try {
      ...
      if (checkIn && checkOut && hasLodgingContext) {
        ...
        if (matched.length > 0) {
          ...
          hotelAvailabilityInstruction = `\n- DISPONIBILITÉ HÔTELS (RÈGLE STRICTE) : L'utilisateur demande des hôtels disponibles du ${checkIn} au ${checkOut} pour ${adults} adulte${adults > 1 ? "s" : ""}. Cite OBLIGATOIREMENT TOUS les ${matched.length} hôtels listés dans la section "DISPONIBILITÉ HÔTELS" ci-dessous (sans exception, ne fais pas de sélection). FORMAT IMPÉRATIF : rédige en PROSE FLUIDE, en paragraphes liés par des connecteurs naturels ("par ailleurs", "à proximité", "dans un autre style", etc.). INTERDICTION ABSOLUE d'utiliser une liste à puces (pas de "-", pas de "*" en début de ligne) ou une liste numérotée (pas de "1.", "2.", "3."). Pour CHAQUE hôtel cité, entoure OBLIGATOIREMENT son nom exact de doubles astérisques (ex. **Riad XYZ**) — c'est indispensable pour que la vignette et le marqueur s'affichent. Indique aussi son prix par nuit en gras précédé de "à partir de" (ex. à partir de **120 EUR/nuit**). INTERDICTION ABSOLUE de mentionner, recommander ou citer d'autres établissements (hôtels, riads, restaurants, lieux à visiter, pépites architecturales, musées, etc.) — ils n'ont pas de disponibilité confirmée et ne sont PAS pertinents pour cette demande. Pas de section "en complément", pas de "à découvrir aussi", pas de suggestions annexes. Termine simplement par une invitation à élargir les dates ou consulter les fiches si besoin.`;
