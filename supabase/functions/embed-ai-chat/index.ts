  ...
  try {
    ...
    const stream = createUIMessageStream({
      execute: async ({ writer }) => {
        ...
        // (by title similarity), emit an ARTICLE_CARD marker AND — when no
        // suggestion-forced badge/subcategory route is active — build a full
        // immersive answer from the article's curated entries (up to 10), so the
        // user gets a real editorial listing instead of a laconic disclosure.
        ...
        try {
          ...
          if (lastUserText && lastUserText.trim().length >= 6) {
            ...
            if (match) {
              ...
              // Only take over the whole response when the current turn is NOT
              // constrained by an initial suggestion route (badges / subcats /
              // pinned IDs / curated proximity). This preserves suggestion-driven
              // behavior while enriching free-text relances that echo an article.
              ...
              if (!suggestionRouteActive) {
                ...
                if (businessIds.length >= 3) {
                  const { data: bizRows } = await admin
                    .from("businesses")
                    .select("id, name, slug, city, neighborhood, main_category, categories, hook_fr, hook_en, hook_ar, description, description_en, description_ar, latitude, longitude, logo_url, images, google_rating, google_review_count, tripadvisor_rating, tripadvisor_review_count, engagements, closure_message, is_active")
                    .in("id", businessIds)
                    .eq("is_active", true)
                    .is("closure_message", null);
                  ...
                  if (orderedBiz.length >= 3) {
                    const limit = Math.min(orderedBiz.length, 10);
                    const shown = orderedBiz.slice(0, limit);

                    // Emit ARTICLE_CARD first so the front renders it at the top.
                    emitDelta(`\n\n<!--ARTICLE_CARD:${JSON.stringify(articlePayload)}-->\n\n`);

                    // Immersive editorial intro (uses the article title).
                    ...
                    // Body: one paragraph per entry, using the curated editorial
                    // copy (pretitle · hook + optional hours), falling back to
                    // the business hook when the entry is thin.
                    const body = shown.map((biz: any, idx: number) => {
                      ...
                      return `**${rank}. ${biz.name}**${area ? ` — _${area}_` : ""}\n\n${detail || (language === "en" ? "A curated One World Morocco address." : language === "ar" ? "عنوان مختار ضمن دليل One World Morocco." : "Une adresse sélectionnée dans le guide One World Morocco.")}${hoursLine}`;
                    }).join("\n\n");
                    ...
                    const disclosure = shown.length < total
                      ...
                      : (language === "en"
                          ? `📍 That's the full **${title}** shortlist in ${cityForCopy} — say the word if you'd like the podium detailed, an alternative neighborhood, or the map view.`
                          : language === "ar"
                    ...
                    // Register businesses so follow-ups (hours, distances, map,
                    // booking) find them via knownBusinesses / SHOW_ON_MAP.
                    ...
                    const mapBusinesses = shown.map((b: any) => ({
                      ...
                      address: null, main_category: b.main_category, categories: Array.isArray(b.categories) ? b.categories : [],
              ...
              if (!blogRouteHandled) {
                // Fallback: keep the legacy behavior — just surface the article card.
                emitDelta(`\n\n<!--ARTICLE_CARD:${JSON.stringify(articlePayload)}-->\n\n`);
              }
            }
          }
