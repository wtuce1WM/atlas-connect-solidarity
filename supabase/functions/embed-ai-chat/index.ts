 ...
 */
function isCityEngagementSearchIntent(text: string): boolean {
  const n = normalize(text || "");
  if (!n) return false;
...
function extractEngagementQueryTerm(text: string): string | null {
  const n = normalize(text || "");
  const patterns = [
...
async function resolveCityEngagementTerm(
  admin: any,
  city: string,
  queryTerm: string,
): Promise<string | null> {
...
async function buildCityEngagementSearch(
  admin: any,
  host: any,
  userMessage: string,
  lang: "fr" | "en" | "ar",
