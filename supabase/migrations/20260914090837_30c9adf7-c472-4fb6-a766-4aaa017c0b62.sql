...
WITH internal AS (
  SELECT DISTINCT
    ...
    COALESCE(d.orientation = 'portrait', false) AS is_portrait,
    false AS is_featured_channel
...
yt AS (
  SELECT DISTINCT
    ...
    COALESCE(y.is_short, false) AS is_portrait,
    true AS is_featured_channel
...
gen AS (
  SELECT DISTINCT
    ...
    (
      ...
      JOIN public.businesses gbb ON gbb.id = gb.business_id
      WHERE gb.generic_video_id = g.id
        AND COALESCE(gbb.is_active, true) = true
    ...
    END::text AS social_url,
    COALESCE(g.orientation = 'portrait', false) AS is_portrait,
    false AS is_featured_channel
  ...
  WHERE COALESCE(g.url, '') <> ''
    AND (
      NOT EXISTS (SELECT 1 FROM public.generic_video_businesses gb WHERE gb.generic_video_id = g.id)
      OR EXISTS (
        SELECT 1 FROM public.generic_video_businesses gb
        JOIN public.businesses gbb ON gbb.id = gb.business_id
        WHERE gb.generic_video_id = g.id AND COALESCE(gbb.is_active, true) = true
      )
    )
    AND (
      _city_ids IS NULL
...
cycled AS (
  SELECT
    gr.*,
    CASE WHEN gr.is_featured_channel
      THEN CEIL(gr.pos_in_group::numeric / 3)::bigint
      ELSE gr.pos_in_group
    END AS cycle_pos
  FROM grouped gr
),
ordered AS (
  SELECT
    c.*,
    row_number() OVER (ORDER BY c.cycle_pos, c.is_portrait DESC, c.h) AS feed_position,
