CREATE OR REPLACE FUNCTION public.get_badges_video_feed(_badge_ids uuid[], _seed text DEFAULT 'owm'::text, _limit integer DEFAULT 60, _offset integer DEFAULT 0, _city_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS TABLE(id text, source text, is_generic boolean, url text, title text, description text, price text, thumbnail_url text, business_id uuid, business_name text, business_logo_url text, business_logo_bg text, social_platform text, social_account text, social_url text, group_key text, feed_position bigint, total_count bigint)
 LANGUAGE sql
...
CREATE OR REPLACE FUNCTION public.get_badge_video_feed(
  _badge_id uuid,
...
GRANT EXECUTE ON FUNCTION public.get_badge_video_feed(uuid, text, integer, integer, uuid[]) TO anon, authenticated, service_role;
