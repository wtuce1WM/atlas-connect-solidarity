...
ALTER TABLE public.businesses
  ADD COLUMN IF NOT EXISTS kayak_url text,
  ADD COLUMN IF NOT EXISTS kayak_rating numeric,
  ADD COLUMN IF NOT EXISTS kayak_review_count integer;
...
BEGIN
  ...
  END IF;
  -- Kayak
  IF NEW.kayak_rating IS NOT NULL AND NEW.kayak_review_count IS NOT NULL AND NEW.kayak_review_count > 0 THEN
    total_count := total_count + NEW.kayak_review_count;
    weighted_sum := weighted_sum + (NEW.kayak_rating / 5.0) * 20.0 * NEW.kayak_review_count;
  END IF;
