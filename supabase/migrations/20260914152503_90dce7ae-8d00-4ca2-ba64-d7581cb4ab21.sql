DROP FUNCTION IF EXISTS public.get_badge_video_feed(uuid, text, integer, integer, uuid[]);
...
CREATE OR REPLACE FUNCTION public.get_badge_video_feed(_badge_id uuid, _seed text DEFAULT 'owm'::text, _limit integer DEFAULT 60, _offset integer DEFAULT 0, _city_ids uuid[] DEFAULT NULL::uuid[])
 RETURNS TABLE(id text, source text, is_generic boolean, url text, title text, description text, price text, thumbnail_url text, business_id uuid, business_name text, business_logo_url text, business_logo_bg text, social_platform text, social_account text, social_url text, group_key text, feed_position bigint)
 LANGUAGE sql
...
yt AS (
  SELECT
    ...
    ytb.name::text AS social_account,
    NULLIF(ytb.youtube_url, '')::text AS social_url
  ...
  LEFT JOIN public.businesses ytb ON ytb.id = y.business_id
  WHERE COALESCE(y.business_is_active, true) = true
    AND COALESCE(y.video_id, '') <> ''
    AND COALESCE(ytb.youtube_channel_featured, false) = true
    AND (
      _city_ids IS NULL
