...
DECLARE
  ...
  v_is_club boolean := COALESCE((NEW.raw_user_meta_data->>'is_club_signup')::boolean, false)
                    OR v_provider IN ('google','apple','microsoft','azure','facebook');
  v_base text;
  v_nick text;
  v_suffix int := 0;
BEGIN
...
-- Rattrapage : enrôler jfshorts1973@gmail.com
INSERT INTO public.club_members (user_id, nickname, email, first_name, last_name)
SELECT
  u.id,
  'jfshorts1973',
  u.email,
...
FROM auth.users u
WHERE u.email = 'jfshorts1973@gmail.com'
  AND NOT EXISTS (SELECT 1 FROM public.club_members cm WHERE cm.user_id = u.id)
ON CONFLICT DO NOTHING;