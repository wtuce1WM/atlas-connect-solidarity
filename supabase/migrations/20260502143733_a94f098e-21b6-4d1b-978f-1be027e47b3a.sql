...
BEGIN
  ...
  -- STEP 1: pre-assign UUIDs to every doc (preserve existing id, otherwise generate one)
  -- This guarantees the same id is used for both business_documents and business_document_cities.
  ...
  IF jsonb_array_length(docs_with_ids) > 0 THEN
    ...
    -- Re-populate business_document_cities using the pre-assigned ids.
