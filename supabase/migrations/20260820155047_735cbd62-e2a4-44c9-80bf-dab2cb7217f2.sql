...
CREATE OR REPLACE FUNCTION public.get_badges_video_feed(
  ...
  _offset integer,
  _city_ids uuid[],
  _include_no_city boolean DEFAULT false
)
RETURNS TABLE(
  id text, source text, is_generic boolean, url text, title text, description text,
  price text, thumbnail_url text, business_id uuid, business_name text,
  business_logo_url text, business_logo_bg text,
  social_platform text, social_account text, social_url text,
  badges jsonb,
  group_key text, feed_position bigint, total_count bigint
)
LANGUAGE sql
STABLE
SECURITY DEFINER
...
GRANT EXECUTE ON FUNCTION public.get_badges_video_feed(uuid[], text, integer, integer, uuid[], boolean) TO anon, authenticated, service_role;