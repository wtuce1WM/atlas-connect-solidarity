...
@media (max-width:640px){
  ...
  const reviewCountInline = totalReviewCount > 0
    ? `<span style="font-family:'Josefin Sans',sans-serif;font-size:1rem;font-weight:500;color:rgba(255,255,255,0.6) !important;margin-left:8px;align-self:center">· ${totalReviewCount.toLocaleString("fr-FR")} ${reviewLabel}</span>`
    : "";
  const ratingBlock = `<div style="display:flex;flex-direction:column;align-items:center;gap:2px;margin-bottom:12px"><div style="display:flex;align-items:center;gap:12px;flex-wrap:wrap;justify-content:center;filter:drop-shadow(0 0 1px hsla(0,0%,0%,0.9)) drop-shadow(0 0 3px hsla(0,0%,0%,0.7)) drop-shadow(0 2px 8px hsla(0,0%,0%,0.5))">${starSvg}<span style="font-family:'Josefin Sans',sans-serif;font-size:3rem;font-weight:900;color:hsl(43,75%,55%) !important">${avgOn20}<span style="font-size:1.5rem;font-weight:600;color:rgba(255,255,255,0.6) !important">/20</span></span>${reviewCountInline}</div></div>`;
