...
const EmbedAsk = ({ paramsOverride }: { paramsOverride?: string } = {}) => {
  ...
  const send = (overrideText?: string, suggestionId?: string, followupId?: string, skipGeoPrompt = false) => {
    ...
    if (!skipGeoPrompt && detectLocalIntent(text)) {
      ...
      try {
        window.postMessage(askedPayload, "*");
        window.postMessage(openPayload, "*");
        if (window.parent && window.parent !== window) {
          window.parent.postMessage(askedPayload, "*");
          window.parent.postMessage(openPayload, "*");
        }
      } catch { /* cross-origin */ }
      return;
    }
  ...
  return (
    ...
    >
      ...
      <div
        ref={scrollRef}
        className={`${autoHeight ? "flex-none" : "flex-1 overflow-y-auto scrollbar-hide"} ${voiceActive ? "[overflow-x:clip]" : ""} px-4 ${heroLayout && !homeState ? "pt-16 md:pt-20" : heroLayout && homeState ? "pt-10 md:pt-4" : "pt-4"} pb-8 space-y-3 ${bg} relative`}
      ...
      >
        ...
        {!homeState && !feedOpening && messages.map((m, i) => {
          ...
          const hasNewerGeoResults = !!geoAnchor && messages.slice(i + 1).some((later) => {
            if (later.role !== "assistant") return false;
            return extractPayloads(messageText(later)).maps.some((payload) => payload.businesses.length > 0);
          });
          ...
          return (
            <div key={m.id || i} className="flex flex-col items-start gap-2">
              ...
              {!hasNewerGeoResults && geoAnchor && mapPayloadRaw && mapPayloadRaw.businesses.length > 0 && (
              ...
              {!hasNewerGeoResults && mapPayload && mapPayload.businesses.length > 0 &&
