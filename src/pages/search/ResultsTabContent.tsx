              ...
              <div className="absolute top-0 left-0 right-0 z-[80] flex flex-col backdrop-blur-sm">
                <div className="flex items-center gap-3 px-3 py-3">
                  ...
                  <span className="text-sm font-medium text-white truncate drop-shadow-md">
                    {mapPoiItems.length} {language === "en" ? "results for" : language === "ar" ? "نتائج لـ" : "résultats pour"} "{activeFsTabId ? `${frontTabs.find(t => t.id === activeFsTabId)?.name || ''} ${effectiveCity || ''}`.trim() : searchQuery}"
                  </span>
                </div>
