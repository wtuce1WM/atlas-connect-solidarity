...
const BookOnlineSlidePanel = ({ businessId: propBusinessId, onClose, externalOverlayActive, forceMuted, interceptCloseRef, showSearchBar, onSearch, onSearchBusinessSelect, onMosaicStateChange, closeTrigger, propagateMosaicState = false }: BookOnlineSlidePanelProps) => {
  ...
  return (
    <div className="h-full overflow-visible overscroll-none bg-black relative">
      ...
      {/* Overlaid content */}
      <div
        className={`relative z-10 flex flex-col ${!showGoogleMap ? 'overflow-hidden' : 'overflow-y-auto'} overflow-x-hidden overscroll-contain h-full p-4 pt-14 md:p-6 md:pt-16 lg:pt-6 ${cardsHidden ? 'pb-0' : 'pb-8'} ${effectiveMedia?.kind === "matterport" ? "pointer-events-none" : externalVideoInteractiveMode ? "pointer-events-none" : ""}`}
                      ...
                      {(descOverlayContent.avgPriceRange || descOverlayContent.priceDetails) && (
                        <div className="flex flex-wrap gap-3 mb-5">
                          ...
                          {descOverlayContent.priceDetails && (
                            <div className="flex-1 min-w-[140px] rounded-xl border border-terracotta/20 px-4 py-3 bg-white">
                              ...
                              <div className="rich-price-html text-sm font-normal leading-relaxed whitespace-pre-line text-terracotta [&_li]:text-base [&_p]:text-base" style={{ fontSize: '0.925rem' }} dangerouslySetInnerHTML={{ __html: descOverlayContent.priceDetails.replace(/([\u{1F300}-\u{1F9FF}\u{2600}-\u{27BF}\u{FE00}-\u{FEFF}\u{1FA00}-\u{1FA6F}\u{1FA70}-\u{1FAFF}\u{2702}-\u{27B0}])/gu, '<span style="font-size:1.5em;line-height:1">$1</span>') }} />
                            </div>
                          )}
                        </div>
                      )}
