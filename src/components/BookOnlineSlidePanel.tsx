...
const BookOnlineSlidePanel = ({ businessId: propBusinessId, onClose, isExpanded, onToggleExpand, externalOverlayActive }: BookOnlineSlidePanelProps) => {
  ...
  const {
    ...
    allVideoUrls, categoryIcon, kpRelated, isKp1Only, liteApiHotelId,
  ...
  return (
    <div className="h-full overflow-hidden overscroll-none bg-black relative">
      ...
      >
        ...
        {/* CTAs + video controls */}
        <div className={`shrink-0 py-2 pb-20 md:pb-14 lg:pb-2 flex flex-col items-center gap-2 pointer-events-auto ${noBottomCarousel ? 'mt-auto' : ''}`}>
            {bookingCta && (
            ...
            {currentMedia?.kind === "video" && videoInfo?.type === "file" && (
              <div className="flex items-center gap-10 mt-5 mb-6 md:mb-4 lg:mb-0 pb-2">
                <button
                  type="button"
                  onClick={() => {
                    if (videoRef.current) {
