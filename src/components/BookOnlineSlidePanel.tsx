    ...
    const fetchData = async () => {
      ...
      if (isKpActive) {
        if (isMaster && kp1Val && kp1Val.trim() !== "") {
          // Principal: show ALL members from KP1 + all businesses referencing this code in KP2
          ...
          <div key={businessId} className="w-full shrink-0 rounded-2xl bg-black/40 backdrop-blur-sm px-4 md:px-6 text-white overflow-hidden relative h-[5.5rem] pointer-events-auto mt-3 md:mt-0 animate-slide-in-right">
            ...
            >
              ...
              <div className="min-w-0 flex-1 text-center md:text-left md:pr-28">
                <h2 className={`text-xl font-bold uppercase${business.name.length > 18 ? ' line-clamp-2 lg:truncate' : ' truncate'}`} style={{ fontFamily: "'Josefin Sans', sans-serif", letterSpacing: '0.12em', WebkitTextStroke: '0.8px currentColor', textShadow: '0 0 0 currentColor' }}>{business.name}</h2>
