...
const BookOnlineSlidePanel = ({ businessId: propBusinessId, onClose, externalOverlayActive, forceMuted, interceptCloseRef, showSearchBar, onSearch, onSearchBusinessSelect, onMosaicStateChange, closeTrigger, propagateMosaicState = false, toolbarPortalPrefix }: BookOnlineSlidePanelProps) => {
  ...
  return (
    <div className="h-full overflow-visible overscroll-none bg-black relative">
      ...
      {/* Overlaid content */}
      <div
        className={`relative z-10 flex flex-col overflow-y-auto overflow-x-hidden overscroll-contain h-full p-4 pt-16 md:p-6 md:pt-20 lg:pt-16 ${cardsHidden ? 'pb-0' : showSearchBar ? 'pb-[70px] md:pb-[66px]' : 'pb-8'} ${(effectiveMedia?.kind === "matterport" && cardsHidden) ? "pointer-events-none" : externalVideoInteractiveMode ? "pointer-events-none" : ""}`}
      ...
      >
        ...
        {(avgOn20 != null && totalReviewCount > 0) || woDescription ? (
          <div className="slidepanel-center-short flex flex-col items-center justify-center pointer-events-auto gap-10 flex-1 -mt-[3rem]">
            {hookText && <TypewriterHook text={hookText} key={businessId + '-hook'} />}
            {woDescription && (
              <div className="mt-24 md:mt-0 slidepanel-plus-short">
                <div
            ...
            {avgOn20 != null && totalReviewCount > 0 && (
              <>
                ...
                <div
                  ...
                  className="flex items-center justify-center gap-2.5 py-1.5 px-4 rounded-full border-2 border-gold cursor-pointer flex-wrap slidepanel-rating-short"
                  onClick={handleOpenReviews}
                ...
                >
                  <div className="flex items-center gap-2.5">
                    <Star className="h-6 w-6 md:h-7 md:w-7 text-gold fill-gold" />
                    <span className="text-3xl md:text-4xl font-black text-gold whitespace-nowrap" style={{ fontFamily: "'Josefin Sans', sans-serif" }}>
                      {avgOn20}<span className="text-lg md:text-xl font-semibold text-white/60">/20</span>
                    </span>
                  </div>
                  <span className="text-xs md:text-sm text-white/60 font-medium whitespace-nowrap" style={{ fontFamily: "'Josefin Sans', sans-serif" }}>
