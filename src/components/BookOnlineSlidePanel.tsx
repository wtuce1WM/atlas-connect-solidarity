...
;
  ...
  return (
    <div className="h-full overflow-visible overscroll-none bg-black relative">
      ...
      {showDescriptionOverlay && (woDescription || descGridSection || descOverlayContent) && (
        <OverlayShell zClass="z-[80]" animClass="animate-zoom-out-center" className="flex flex-col">
          ...
          {!descGridSection && (() => {
            ...
            const hasReservation = externalLinks.length > 0;
                ...
                <div className="flex items-center gap-2 px-3 py-1.5 bg-black/40 backdrop-blur-sm border-t border-white/10 overflow-x-auto scrollbar-none">
                  ...
                  {externalLinks.map((link) => (
                    <button
                      ...
                      className={`shrink-0 h-8 w-8 flex items-center justify-center rounded-full overflow-hidden transition-colors ${link.icon ? 'bg-white hover:bg-white/90' : 'bg-white/15 hover:bg-white/30 text-white'}`}
                      title={link.name || 'Lien'}
                    >
                      {link.icon ? (
                        <img src={link.icon} alt={link.name || ''} className="h-8 w-8 object-contain rounded-full p-1" loading="lazy" />
                      ) : (
                        <span className="text-[10px] font-medium uppercase font-['Josefin_Sans',sans-serif]">{(link.name || 'L').slice(0, 2)}</span>
                      )}
                    </button>
                  ))}
                  {(socialItems.length > 0 || externalLinks.length > 0) && menuDocs.length > 0 && (
      ...
      {/* Search bar */}
      {showSearchBar && !docOverlay && !cardsHidden && !showDescriptionOverlay && (
        <PanelSearchBar
