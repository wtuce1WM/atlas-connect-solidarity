          ...
          <div className="flex-1 flex items-start justify-center overflow-hidden min-h-0">
            <div className="w-[95%] md:w-[90%] lg:w-[70%] max-h-full overflow-hidden rounded-2xl bg-black/40 backdrop-blur-sm p-4 md:p-6 flex flex-col gap-5 text-white pointer-events-auto">
              ...
              {woDescription && descExpanded && (
                <div
                  className="min-h-0 max-h-16 md:max-h-20 overflow-y-auto pr-1 text-sm leading-relaxed prose prose-invert prose-sm max-w-none break-words [&_*]:!text-white [&_a]:!text-white/90 [&_a:hover]:!text-white"
              ...
              <div className="flex gap-2 overflow-x-auto px-4 pb-1 scrollbar-hide">
                {destinations.map((dest) => {
                  ...
                  return (
                    <div
                      key={dest.id}
                      className="shrink-0 w-28 rounded-xl overflow-hidden bg-black/40 backdrop-blur-sm border border-white/10"
                    >
                      {destImg ? (
                        <img src={destImg} alt={destName(dest)} className="w-full h-16 object-cover" />
                      ) : (
                        <div className="w-full h-16 bg-white/10 flex items-center justify-center">
                          <MapPin className="h-5 w-5 text-white/40" />
                        </div>
                      )}
                      <p className="text-[10px] font-medium text-white text-center py-1 px-1 truncate">
                        {destName(dest)}
                      </p>
                    </div>
                  );
