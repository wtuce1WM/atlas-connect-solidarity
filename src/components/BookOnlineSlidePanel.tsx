...
const BookOnlineSlidePanel = ({ businessId: propBusinessId, onClose, isExpanded, onToggleExpand, externalOverlayActive, forceMuted }: BookOnlineSlidePanelProps) => {
  ...
  return (
    <div className="h-full overflow-visible overscroll-none bg-black relative">
      ...
      <div
        className={`relative z-10 flex flex-col overflow-y-auto overscroll-contain h-full p-4 pt-12 md:p-6 md:pt-16 lg:pt-6 ${cardsHidden ? 'pb-0' : 'pb-8'}`}
            ...
            {fallbackPanelData && !hotelSearchLoading && (() => {
              ...
              return (
                <div className="flex flex-col items-center justify-center flex-1 w-full px-4 md:px-0">
                  {/* Contextual message */}
                  <div className="text-left text-white bg-black/40 backdrop-blur-sm rounded-xl px-5 py-4 border border-white/10 w-full md:w-auto md:max-w-none" style={{ maxWidth: 'calc(100% - 0px)' }}>
                  ...
                  {fallbackPanelData.hotels.filter(h => !h.isCurrentHotel).length > 0 && (
                    <div className="text-center text-white bg-black/40 backdrop-blur-sm rounded-xl px-5 py-3 border border-white/10 mt-3 font-['Roboto',sans-serif] cursor-pointer hover:bg-black/50 transition-colors" onClick={() => { setShowFallbackOverlay(true); }}>
                      ...
                      <p className="text-[16px] text-white/80 mt-1.5 underline underline-offset-2">
