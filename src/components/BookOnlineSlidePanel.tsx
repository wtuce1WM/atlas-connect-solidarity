          ...
          <div key={businessId} className="w-full shrink-0 rounded-2xl bg-black/40 backdrop-blur-sm px-4 md:px-6 text-white overflow-hidden relative h-[5.5rem] pointer-events-auto mt-3 md:mt-0 animate-slide-in-right">
            ...
            >
              ...
              <div className="min-w-0 flex-1 text-center md:text-left md:pr-28">
                <h2 className={`text-xl font-bold uppercase${business.name.length > 18 ? ' line-clamp-2 md:truncate' : ' truncate'}`} style={{ fontFamily: "'Josefin Sans', sans-serif", letterSpacing: '0.12em', WebkitTextStroke: '0.8px currentColor', textShadow: '0 0 0 currentColor' }}>{business.name}</h2>
                {(business.city || business.neighborhood) ? (
                  <p className={`text-xs md:text-sm text-white/80 flex items-center gap-1 mt-0.5 justify-center md:justify-start${business.name.length > 18 ? ' hidden md:flex' : ''}`}>
