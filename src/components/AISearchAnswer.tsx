...
const BusinessHoverCard = ({ name, business, onClickBusiness }: { name: string; business: BusinessData; onClickBusiness: (b: BusinessData) => void }) => {
  ...
  return (
    <HoverCard openDelay={200} closeDelay={100}>
      ...
      <HoverCardContent side="bottom" align="center" avoidCollisions sideOffset={8} className="z-[100] w-72 p-0 overflow-hidden rounded-xl border border-gold/20 shadow-xl">
        {img && (
          ...
          <div className="px-5 py-4">
            {isLoading ? (
              ...
              <div className={`leading-relaxed text-foreground transition-all duration-200 ${
                ...
                {formatAnswer(answer, businesses, setSelectedBusiness, highlightWordIndex, fadeWordIndex, fadeComplete)}
              </div>
            )}
          </div>
        </div>
