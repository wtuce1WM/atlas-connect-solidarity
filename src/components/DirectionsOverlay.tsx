...
/* ── Terracotta pin icon (same Pin used elsewhere) ── */
...
const DirectionsOverlay = ({ business, onClose }: DirectionsOverlayProps) => {
  ...
  const showMap = !!origin && !needsGeoConsent && !waitingForOrigin;

  // Measure info card to add padding when fitting the route
  useEffect(() => {
    if (!showInfoCard) {
      setCardOffset(0);
      return;
  ...
  // Init map
  useEffect(() => {
    if (!mapsReady || !showMap || !mapDivRef.current || mapRef.current) return;
    const gmaps = window.google.maps;
    mapRef.current = new gmaps.Map(mapDivRef.current, {
      center: origin ?? { lat: 31.63, lng: -7.98 },
      zoom: 13,
    ...
    rendererRef.current = new gmaps.DirectionsRenderer({
      map: mapRef.current,
  ...
  // Compute & render route + markers
  useEffect(() => {
    if (!mapsReady || !showMap || !mapRef.current || !rendererRef.current || !origin) return;
    const gmaps = window.google.maps;
    ...
    originMarkerRef.current = new gmaps.Marker({
      position: origin,
      map,
      icon: buildPinIcon(gmaps),
      title: "Vous êtes ici",
      zIndex: 2000,
    });

    // Destination marker (default Google red)
    ...
    service.route(
      {
        origin,
        destination: destLatLng ?? destRaw,
      ...
      (result, status) => {
        ...
        if (status === "OK" && result) {
          ...
          // Fallback: just center on origin
  ...
  }, [mapsReady, showMap, origin, business.latitude, business.longitude, business.name, destRaw, directionsMode, cardOffset]);
