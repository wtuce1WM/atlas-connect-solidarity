      ...
      <div ref={scrollContainerRef} className="flex-1 overflow-y-auto relative">
        ...
        {showStickyHeader && (
          <div className="sticky top-0 z-20 bg-background/95 backdrop-blur-sm border-b border-border px-4 py-2.5 flex items-center gap-3 animate-in fade-in-0 slide-in-from-top-2 duration-200">
            ...
            {(() => {
              const socials = [
                { url: business.facebook_url, color: "#1877F2", icon: <FacebookIcon className="h-4 w-4" /> },
                { url: business.instagram_url, color: "#E4405F", icon: <InstagramIcon className="h-4 w-4" /> },
                { url: business.linkedin_url, color: "#0A66C2", icon: <LinkedInIcon className="h-4 w-4" /> },
                { url: business.youtube_url, color: "#FF0000", icon: <YouTubeIcon className="h-4 w-4" /> },
                { url: business.tiktok_url, color: "#000000", icon: <TikTokIcon className="h-4 w-4" /> },
                { url: business.twitter_url, color: "#000000", icon: <TwitterIcon className="h-4 w-4" /> },
                { url: business.pinterest_url, color: "#E60023", icon: <PinterestIcon className="h-4 w-4" /> },
                { url: business.vimeo_url, color: "#1AB7EA", icon: <VimeoIcon className="h-4 w-4" /> },
              ].filter(s => s.url);
                ...
                <div className="grid grid-cols-2 gap-4">
                  {socials.length > 0 && (
                    <div className="space-y-2">
                      <h3 className="text-sm font-bold text-muted-foreground uppercase tracking-wide">Réseaux sociaux</h3>
