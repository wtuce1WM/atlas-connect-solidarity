  ...
  return (
    <div className="flex flex-col h-full relative">
            ...
            {(
              ...
              <div ref={mediaContainerRef} className={`relative w-full aspect-square sm:aspect-[16/9] bg-muted ${mediaCount > 1 ? "cursor-pointer" : ""}`} onClick={() => { if (mediaCount > 1 && !(hasVideo && currentImageIndex === 0)) setIsLightboxOpen(true); }}>
                ...
                {isVerified && !isInstitution && (
                  <img src={logoGold} alt="WTUCE" className="absolute top-3 right-3 w-[4.5rem] h-[4.5rem] md:w-[7rem] md:h-[7rem] lg:w-36 lg:h-36 object-contain opacity-90 pointer-events-none drop-shadow-lg" />
                )}
                {mediaCount > 1 && (
                  <>
                    <button
      ...
      {fallbackPanelData && createPortal(
        <div className={leftPanelPortalRef?.current ? "absolute inset-0 z-10 flex" : "<div className={leftPanelPortalRef?.current ? "absolute inset-0 z-10 flex" : "fixed inset-0 z-[200] flex flex-col justify-end lg:justify-start lg:right-auto lg:w-1/2"} style={leftPanelPortalRef?.current ? undefined : { top: "53px" }}>"} style={leftPanelPortalRef?.current ? undefined : { top: "53px" }}>
