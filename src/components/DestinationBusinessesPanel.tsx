...
const DestinationBusinessesPanel = ({ destination, language, onClose, onBusinessClick, onLoginRequired }: DestinationBusinessesPanelProps) => {
  ...
  return (
    ...
    {isExpanded && (
      ...
      <div
        className="fixed top-[48px] left-0 bottom-0 z-[39] bg-black/40 backdrop-blur-[2px]"
        style={{ width: "20%" }}
        onClick={() => setIsExpanded(false)}
      />
      <div className="fixed top-[48px] right-0 bottom-0 z-[40] bg-background flex flex-col border-l-2 border-border shadow-[-8px_0_30px_-5px_rgba(0,0,0,0.15)]" style={{ width: "80%" }}>
    ...
    {!isExpanded && (
    <div className="fixed top-[48px] right-0 bottom-0 z-40 border-l border-border bg-background flex flex-col shadow-2xl transition-all duration-500 ease-out w-1/2">
      {/* Header */}
