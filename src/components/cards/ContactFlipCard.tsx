...
const ContactFlipCard = ({
  ...
  const frontHeight = showHotel ? "6.5em" : "6.5em";
  // Back height for hotel calendar
  const hotelBackHeight = tallHeight ? "28em" : "26em";

  return (
    <div
      className={`snap-start shrink-0 ${showHotel ? 'w-[20rem]' : 'w-fit min-w-[14rem]'} mb-4 rounded-2xl bg-black/40 backdrop-blur-sm border border-white/10 animate-slide-in-left opacity-0 transition-[height] duration-500 ease-in-out ${className}`}
      style={{
        perspective: "1000px",
        animationDelay,
        animationFillMode: "forwards",
        height: showHotel
          ? (flipped ? hotelBackHeight : frontHeight)
    ...
    >
      ...
      >
        ...
        {/* ─── BACK — Hotel calendar (hotel mode) ─── */}
        {showHotel && (
          <div
            className="absolute inset-0 rounded-2xl p-3 text-white flex flex-col"
