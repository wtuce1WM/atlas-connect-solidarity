...
const HomepageCardsFront = ({ city, onLabelClick, labelTakesPriority = false }: Props) => {
  ...
  useEffect(() => {
    const firstThumb = optimizeSupabaseImage(slots[0]?.data.thumbnail, { width: 300, quality: 60 });
  ...
  const renderCard = (slot: MixedSlot, index: number) => {
    ...
    const optimizedThumb = optimizeSupabaseImage(it.thumbnail, isPriority ? { width: 300, quality: 60 } : { width: 400 });
