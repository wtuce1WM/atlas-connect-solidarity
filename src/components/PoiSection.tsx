...
const PoiSection = ({ city, language, onBusinessClick, columns, onMapClick, onPoisLoaded, onHover }: PoiSectionProps) => {
  ...
  return (
    <>
      <div className={`grid gap-4 pt-10 <div className={`grid gap-4 pt-10 sm:pt-12 lg:pt-14 pb-28 [overflow-anchor:none] ${columns === 2 ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"}`}> pb-28 [overflow-anchor:none] ${columns === 2 ? "grid-cols-1 lg:grid-cols-2" : "grid-cols-1 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4"}`}>
        {pois.map((biz) => {
