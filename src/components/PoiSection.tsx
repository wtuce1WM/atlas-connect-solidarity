...
const PoiSection = ({ city, language, onBusinessClick, columns, onMapClick, onPoisLoaded, onHover }: PoiSectionProps) => {
  ...
  return (
    <>
      <div className={`pt-20 pb-28 ${columns === 3 ? "grid grid-cols-1 sm:grid-cols-3 gap-3" : columns === 2 ? "grid grid-cols-1 sm:grid-cols-2 gap-3" : "grid grid-cols-1 sm:grid-cols-2 gap-3 lg:grid-cols-4"}`}>
        {pois.map((biz) => {
